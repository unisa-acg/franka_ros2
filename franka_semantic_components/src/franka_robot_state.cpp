...
auto FrankaRobotState::get_values_as_message(franka_msgs::msg::FrankaRobotState& message) -> bool {
  ...
  if (franka_state_interface != state_interfaces_.end()) {
    {
      auto opt = (*franka_state_interface).get().get_optional();
      if (!opt.has_value()) {
        RCLCPP_ERROR(rclcpp::get_logger("franka_state_semantic_component"),
                     "Franka state interface value missing! Did you assign the loaned state in the "
                     "controller?");
        return false;
      }
      robot_state_ptr = bit_cast<franka::RobotState*>(opt.value());
    }
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("franka_state_semantic_component"),
